    ...
    <li>
      <article className="movie-list__item">
        <div className="movie-list__item-img">
          <img onError={handleOnImageError} src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt="" />
        </div>
        <header className="movie-list__item-header">
          <h2 className="movie-list__item-title">{movie.title}</h2>
          {movie.release_date && <span className="movie-list__item-release">{format(movie.release_date, "PP")}</span>}
          <div className="movie-list__item-genres">
            {movie.genre_ids.map((id) => (
              <MovieGenre key={id} genre={genresQuery.data.find((genre) => genre.id === id)?.name || "unknown"} />
            ))}
          </div>
